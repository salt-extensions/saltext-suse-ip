...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/suse_ip/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/suse_ip/*")
            ...
            #    "--include=src/saltext/suse_ip/*,tests/*"
            # )
        finally:
